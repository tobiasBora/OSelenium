(* https://github.com/SeleniumHQ/selenium/blob/master/py/selenium/selenium.py *)

let sp = Printf.sprintf
exception BadDataFormat of string
let (>>=) = Lwt.(>>=)
let (>|=) = Lwt.(>|=)
			     
(** Defines an object that runs Selenium commands*)
class selenium ~host ~port ~browserStartCommand ?(http_timeout=90.) ~browserURL =
object (self)
  val mutable m_host = host
  val mutable m_port = port
  val mutable m_browserStartCommand = browserStartCommand
  val mutable m_browserURL = browserURL
  val mutable m_sessionId = None
  val mutable m_extensionJs = ""
  val mutable m_http_timeout = http_timeout

  method setExtensionJs extensionJs = m_extensionJs <- extensionJs

  method start ?browserConfigurationOptions ?(driver:string option) () =
    let start_args =
      [m_browserStartCommand; m_browserURL; m_extensionJs]
      @ (match browserConfigurationOptions with
	   None -> []
	 | Some x -> [sp "webdriver.remote.sessionid=%s" x])
      @ (match driver with
	   None -> []
	 | Some x -> [sp "webdriver.remote.sessionid=%s" x])
    in
    (* Sessions can take a while to start, specially in grid environments *)
    m_http_timeout <- m_http_timeout *. 5.;
    lwt result = self#get_string' "getNewBrowserSession" start_args in
    m_http_timeout <- m_http_timeout *. 5.;
    m_sessionId <- result;
    Lwt.return ()

  method get_string' verb args =
    self#do_command' verb args
    >|= fun data ->
	String.sub data 2 (String.length data - 2)
  ...
  method do_command' verb args =
    let body =
      args
      |> List.mapi (fun i a -> sp "%d=%s" (i+1) (Uri.pct_encode a))
      |> (fun l -> match m_sessionId with
		     None -> l
		   | Some x -> l @ [sp "sessionId=%s" x])
      |> List.fold_left
	   (fun a b -> a ^ "&" ^ b)
	   (sp "cmd=%s" (verb |> Uri.pct_encode))
    ...
    let uri = Uri.make
		...
		()
    in
    (* TODO : Find a better way than Lwt.pick ? *)
    Lwt.pick [Cohttp_lwt_unix.Client.post ~body ~headers uri;
	      Lwt_unix.timeout m_http_timeout]
    >>= fun (response,body) ->
    Cohttp_lwt_body.to_string body
    >>= fun (data) ->
    if data.[0] = 'O' && data.[1] = 'K' then
      Lwt.return data
    else Lwt.fail (BadDataFormat data)
end;;
